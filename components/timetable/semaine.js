...
const Semaine = forwardRef((props, ref) => {
  ...
  const filterEventsByDate = useCallback((events, centerDate) => {
  ...
  const handleDateChange = useCallback((date) => {
    ...
    // Log pour debug
    console.log(`Loaded ${filteredEvents.length} events for ${WEEKS_TO_PRELOAD * 2 + 1} weeks`);
  ...
  const EventComponent = useMemo(() => React.memo(({ event }) => {
    const isLongEvent = event._internal.duration > 600;
    const eventStyle = isLongEvent ? styles.eventContainerLong : styles.eventContainerShort;
    const titleStyle = isLongEvent ? styles.eventTitleLong : styles.eventTitleShort;
  ...
  const calendarTheme = useMemo(() => ({
    ...
    nowIndicatorColor: colors.white,
  }), [colors]);
  ...
  const calendarProps = useMemo(() => ({
    ...
    isShowHalfLine: true,
    initialTimeIntervalHeight: Dimensions.get("screen").height / 17.7 + (Platform.OS === "android" ? 1 : 0),
    theme: calendarTheme
